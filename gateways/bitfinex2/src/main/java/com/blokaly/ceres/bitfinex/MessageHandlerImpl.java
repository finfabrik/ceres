...
public class MessageHandlerImpl implements MessageHandler {
    ...
    public void onMessage(InfoEvent event) {
        ...
        if (version != null) {
            ...
            if (!"2".equals(vers[0])) {
                LOGGER.error("Unsupported version: {}, only v2 supported.", version);
                return;
            }
            enableSequence();
        } else {
    ...
    @Override
    public void onMessage(ConfEvent event) {
        LOGGER.info("Received conf {}", event);
        if (event.isOK()) {
            subscribeAll();
        } else {
            LOGGER.error("Conf error");
            System.exit(1);
        }
    }
    ...
    private void enableSequence() {
        BitfinexClient sender = clientProvider.get();
        if (sender == null) {
            LOGGER.error("Bitfinex client unavailable, skip sending conf");
            return;
        }

        String jsonString = gson.toJson(new ConfEvent().enableSeqence());
        LOGGER.info("conf: {}", jsonString);
        sender.send(jsonString);
    }
}
